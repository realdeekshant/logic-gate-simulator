...
class SimulatorSubWindow(NodeEditorWidget):
    def __init__(self):
        super().__init__()
    ...
    def doEvalOutputs(self):
        ...
        for node in self.scene.nodes:
            if node.__class__.__name__ == "GateNode_Output":
                node.eval()
    ...
    # helper functions
    def determine_target_socket_of_node(self, was_dragged_flag, new_gate_node):
        target_socket = None
        if was_dragged_flag:
            if len(new_gate_node.inputs) > 0: target_socket = new_gate_node.inputs[0]
        else:
            if len(new_gate_node.outputs) > 0: target_socket = new_gate_node.outputs[0]
        return target_socket

    def finish_new_node_state(self, new_gate_node):
        self.scene.doDeselectItems()
        new_gate_node.grNode.doSelect(True)
        new_gate_node.grNode.onSelected()
    ...
    def handleNewNodeContextMenu(self, event):
        ...
        if action is not None:
            new_gate_node = get_class_from_opcode(action.data())(self.scene)
            scene_pos = self.scene.getView().mapToScene(event.pos())
            new_gate_node.setPos(scene_pos.x(), scene_pos.y())
            if DEBUG_CONTEXT: print("Selected node:", new_gate_node)

            if self.scene.getView().mode == MODE_EDGE_DRAG:
                # if we were dragging an edge...
                target_socket = self.determine_target_socket_of_node(self.scene.getView().dragging.drag_start_socket.is_output, new_gate_node)
                if target_socket is not None:
                    self.scene.getView().dragging.edgeDragEnd(target_socket.grSocket)
                    self.finish_new_node_state(new_gate_node)

            else:
                self.scene.history.storeHistory("Created %s" % new_gate_node.__class__.__name__)
